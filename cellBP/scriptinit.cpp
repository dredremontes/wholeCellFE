...
//#include <invagination.h> not needed 
#include <fstream>
...
void ScriptInit::parse_script( const char* filename )
{
    ...
    if ( ecomsol_branch != NULL )
    {
        ...
        // NOTE: concentrations set to zero
        std::vector<double> in_c_A(obj_vertices.size(),0);
        std::vector<double> in_c_B(obj_vertices.size(),0);
        std::vector<double> in_c_C(obj_vertices.size(),0);
        // Deprecated: read the mesh and the concentrations 
        //read_ecomsol( trimesh, obj_vertices,in_c_B,in_c_C,in_c_BC, meshpath.c_str() );        
        ...
        // NOTE this function also appends concentrations
        append_mesh( triangles, vertices, masses, c_A, c_B, c_C, obj_triangles, obj_vertices, obj_masses, in_c_A, in_c_B, in_c_C );

        // NOTE: velocity and strains, appending directly
        V_t.clear();
        A_t.clear();
        ea_t.clear();
        fa_u.clear();
        ...
        for(int ni=0;ni<vertices.size();ni++){
            V_t.push_back(Vec3d(0.0,0.0,0.0));
