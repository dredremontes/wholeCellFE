...
size_t SurfTrack::add_vertex( const Vec3d& new_vertex_position, double new_vertex_mass, double new_vertex_cA, double new_vertex_cB,double new_vertex_cC, const Vec3d& new_vertex_Vt, const Vec3d& new_vertex_At, const Eigen::VectorXd& new_vertex_eat, const Vec3d& new_vertex_fau)
{
    ...
    if( new_vertex_index > get_num_vertices() - 1 )
    {
        ...
        m_cA.resize( new_vertex_index  + 1 );
        m_cB.resize( new_vertex_index  + 1 );
        m_cC.resize( new_vertex_index  + 1 );
        m_Vt.resize(new_vertex_index  + 1 );
        m_At.resize(new_vertex_index  + 1 );
        m_eat.resize(new_vertex_index  + 1 );
        m_fau.resize(new_vertex_index  + 1 );
    }
    ...
    // NOTE: changing also for concentrations
    m_cA[new_vertex_index] = new_vertex_cA;
    m_cB[new_vertex_index] = new_vertex_cB;
    m_cC[new_vertex_index] = new_vertex_cC;

    // changing the strains, velocities, accelerations 
    m_Vt[new_vertex_index] = new_vertex_Vt;
    ...
    else
    {
        ...
        {      
            if ( !m_mesh.vertex_is_deleted(i) )
            {
                ...
                // NOTE: changing also for velocities, strains
                m_Vt[j] = m_Vt[i];
                m_At[j] = m_At[i];
                m_eat[j] = m_eat[i];
        ...
        // NOTE: also change for velocities, etc...
        m_Vt.resize(j);
        m_At.resize(j);
        m_eat.resize(j);
        m_fau.resize(j);
